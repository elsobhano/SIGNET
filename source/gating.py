...
def main(args):
    ...
    logger_config = {
        "logging": {
            "project_name": f"{args.dataset}--{args.task}",
            "run_name": f"signet-{args.dataset}-{args.task}",
    ...
    total_params = sum(p.numel() for p in gate_model.parameters() if p.requires_grad)
    print(f"Total trainable parameters in gate model: {total_params}")
    # exit(0)
    ...
    if args.eval:
        if utils.is_main_process():
            if args.task != "ISLR":
                print("📄 dev result")
                evaluate(args, dev_dataloader, gate_model, model_without_ddp, experts,phase='dev')
            print("📄 test result")
            evaluate(args, test_dataloader, gate_model, model_without_ddp, experts, phase='test')
    ...
    for epoch in tqdm(range(0, args.epochs)):
        ...
        if utils.is_main_process():
            if args.dataset != 'How2Sign':
                ...
                test_stats = evaluate(args, test_dataloader, gate_model, model_without_ddp, experts, phase='test')
            ...
            if args.task == "SLT":
                ...
                if max_accuracy < test_stats["bleu4"]:
                    ...
                    for old_checkpoint in output_dir.glob('best_checkpoint_b4_*.pth'):
                        old_checkpoint.unlink()
                    
                    # Update max_accuracy and save new best checkpoint
                    max_accuracy = test_stats["bleu4"]
                    best_checkpoint_path = output_dir / f'best_checkpoint_b4_{max_accuracy:.2f}.pth'
                    utils.save_on_master({
                        'model': get_requires_grad_dict(model_without_ddp),
                        'epoch': epoch,
                        'bleu4': max_accuracy,
...
def train_one_epoch(args, model, experts, data_loader, optimizer, epoch):
    ...
    for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        # if step < 5:  # Skip first few batches for warmup
        #     continue
        start = time.time()
        ...
        if num_batches >= 100:  # Measure 100 batches
            break
    ...
    throughput = num_batches * 8 / total_time
    print(f"Average latency per batch: {avg_latency:.4f}s")
...
def evaluate(args, data_loader, model, model_without_ddp, experts, phase):
    ...
    with torch.no_grad():
        ...
        tgt_refs = []
        all_ref_lens = []
        all_gen_token_lens = []   # lengths before decoding
        ...
        for step, (src_input, tgt_input) in enumerate(metric_logger.log_every(data_loader, 10, header)):
            ...
            output = model_without_ddp.generate(gate_out, 
                                                max_new_tokens=100, 
                                                num_beams = 4,
                        )
            ...
            for i in range(len(output)):
                ...
                all_ref_lens.append(len(tgt_input['gt_sentence'][i].split()))
                all_gen_token_lens.append(len(output[i]))

            # Running averages (per batch)
            if step % 10 == 0:
                avg_ref_len = np.mean(all_ref_lens)
                avg_gen_len = np.mean(all_gen_token_lens)
                print(f"[Eval Step {step}] Avg Ref Len: {avg_ref_len:.1f} | Avg Gen Token Len: {avg_gen_len:.1f}")
    ...
    elif args.dataset == 'Phoenix' and args.task == "SLT":
        tgt_pres = [r.replace("\n",'') + ' .' for r in tgt_pres]
        tgt_refs = [r.replace("\n",'') + ' .' for r in tgt_refs]
    elif args.dataset == 'How2Sign' and args.task == "SLT":
        tgt_pres = [r.replace("\n",'') for r in tgt_pres]
        tgt_refs = [r.replace("\n",'') for r in tgt_refs]
    
    # --- Length diagnostics after decoding ---
    gen_word_lens = [len(p.split()) for p in tgt_pres]
    ref_word_lens = [len(r.split()) for r in tgt_refs]
    avg_ref_len = np.mean(ref_word_lens)
    avg_gen_len = np.mean(gen_word_lens)
    short_ratio = sum(gl < 0.6 * rl for gl, rl in zip(gen_word_lens, ref_word_lens)) / len(ref_word_lens)

    print(f"{phase}=" * 50)
    print(f"[EVAL SUMMARY] Avg Ref Len: {avg_ref_len:.1f} | Avg Gen Len: {avg_gen_len:.1f}")
    print(f"[EVAL SUMMARY] Min/Max Gen Len: {np.min(gen_word_lens):.0f}/{np.max(gen_word_lens):.0f}")
    print(f"[EVAL SUMMARY] % Gens shorter than 60% of ref: {short_ratio * 100:.1f}%")
    print("=" * 50)

    # --- Log to metric logger ---
    metric_logger.meters['avg_ref_len'].update(avg_ref_len)
    metric_logger.meters['avg_gen_len'].update(avg_gen_len)
    metric_logger.meters['min_gen_len'].update(np.min(gen_word_lens))
    metric_logger.meters['max_gen_len'].update(np.max(gen_word_lens))
    metric_logger.meters['short_ratio'].update(short_ratio)
    ...
    if args.task == "SLT":
        print(len(tgt_refs), len(tgt_pres))
    ...
    if utils.is_main_process() and utils.get_world_size() == 1 and args.eval:
        with open(args.output_dir+f'/{phase}_gating_pres.txt','w') as f:
            for i in range(len(tgt_pres)):
                f.write(tgt_pres[i]+'\n')
        with open(args.output_dir+f'/{phase}_gating_refs.txt','w') as f:
...
if __name__ == '__main__':
    ...
    args.output_dir = os.path.join(args.output_dir, args.dataset, 'gating', timestamp_str)
